...
export function getConfig(): CliConfig {
    ...
    let baseUrl = env.KOB_API_BASE_URL || process.env.KOB_API_BASE_URL || 'https://www.kob-ai.dev';
    ...
    const rawKey = env.KOB_API_KEY || process.env.KOB_API_KEY || '';
    const modelId = env.KOB_MODEL_ID || process.env.KOB_MODEL_ID;

    if (!rawKey) {
        console.error('❌ Error: KOB_API_KEY environment variable is required');
        console.error(`Please set it in ${describeEnvPath()} or run "kob config"`);
